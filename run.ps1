...
Start-Process powershell -ArgumentList "-NoExit","-Command","Set-Location -LiteralPath '$backend'; if(-not (Test-Path '.\venv')){ python -m venv .\venv }; . .\venv\Scripts\Activate.ps1; pip install -r requirements.txt; python -m uvicorn main:app --reload --host 0.0.0.0 --port 8000" -WorkingDirectory $backend
